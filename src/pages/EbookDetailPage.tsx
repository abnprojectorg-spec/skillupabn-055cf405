...
const EbookDetailPage = () => {
  ...
  useEffect(() => {
    if (user && id) {
      hasEbookAccess(user.uid, id).then(setPurchased);
    }
  }, [user, id]);

  const existingPayment = payments.find(
    (p) => p.ebookId === id && (p.status === "pending" || p.status === "approved")
  );
  ...
  const handleSubmitPayment = async () => {
    ...
    if (!user || !ebook || !id) { toast({ title: "Please log in first", variant: "destructive" }); return; }

    setSubmitting(true);
    try {
      await submitEbookPaymentRequest({
        userId: user.uid,
  ...
  const handleDownloadQR = () => {
    if (!ebook?.qrCodeUrl) return;
    const link = document.createElement("a");
    link.href = ebook.qrCodeUrl;
    link.download = `${ebook.title}-telebirr-qr.png`;
    link.target = "_blank";
    link.click();
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="pt-20 pb-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <Link to="/ebooks" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-accent transition-colors mb-6">
            <ArrowLeft className="h-4 w-4" /> Back to Ebooks
          </Link>

          <div className="grid gap-8 lg:grid-cols-3">
            <div className="lg:col-span-2">
              <h1 className="font-display text-3xl font-bold mb-3">{ebook.title}</h1>
              <p className="text-muted-foreground mb-4">by <span className="text-foreground font-medium">{ebook.author}</span></p>

              <div className="flex flex-wrap items-center gap-4 text-sm text-muted-foreground mb-6">
                <span className="flex items-center gap-1"><FileText className="h-4 w-4" /> {ebook.pages} pages</span>
                <span className="flex items-center gap-1"><User className="h-4 w-4" /> {ebook.author}</span>
              </div>

              <div className="aspect-[3/4] max-w-sm overflow-hidden rounded-xl bg-card border border-border mb-8">
                <img src={ebook.coverImage || "/placeholder.svg"} alt={ebook.title} className="h-full w-full object-cover" />
              </div>

              <h2 className="font-display text-xl font-semibold mb-3">About This Ebook</h2>
              <p className="text-muted-foreground leading-relaxed mb-6 whitespace-pre-wrap">{ebook.description}</p>

              {ebook.whatYouWillLearn && (
                <>
                  <h2 className="font-display text-xl font-semibold mb-3">What You'll Learn</h2>
                  <div className="space-y-2 mb-6">
                    {ebook.whatYouWillLearn.split("\n").filter(Boolean).map((item, i) => (
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 rounded-2xl border border-border bg-card p-6">
                ...
                {purchased ? (
                  <Link to={`/read-ebook/${id}`}>
                    <Button variant="hero" size="lg" className="w-full mb-3 shadow-glow">
                      <BookOpen className="h-4 w-4 mr-1" /> Read Ebook
                    </Button>
                  </Link>
                ...
                <ul className="space-y-3 text-sm text-muted-foreground">
                  <li className="flex items-center gap-2"><FileText className="h-4 w-4" /> {ebook.pages} pages</li>
                  <li className="flex items-center gap-2"><BookOpen className="h-4 w-4" /> PDF format</li>
                  <li className="flex items-center gap-2"><Download className="h-4 w-4" /> Downloadable</li>
                </ul>
              </div>
            </div>
          </div>
            ...
            {submitted ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground leading-relaxed">
                  Payment submitted successfully. Verification may take up to 5 hours. Your ebook will unlock once approved.
                </p>
